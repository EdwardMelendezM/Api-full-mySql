...
const updateLanguage = async (req, res) => {
  try {
    ...
    if (id === undefined || name === undefined || programmers === undefined) {
      res.status(400).json({ message: "DATA_UNDEFINED_UPDATE" })
    }
    const language = { name, programmers };
    const connection = await getConnection();
    const result = await connection.query("UPDATE language SET ? WHERE id=?", [language, id]);
    res.send({ message: "DATA_COMPLETE_UPDATED" })
  } catch (error) {
    res.status(500);
    res.send(error.message);
  }
...
const deleteLanguage = async (req, res) => {
  try {
    const { id } = req.params;
    if (id === undefined) {
      res.send({ message: "DATA_UNDEFINED_TO_DELETE" })
    }
    const connection = await getConnection();
